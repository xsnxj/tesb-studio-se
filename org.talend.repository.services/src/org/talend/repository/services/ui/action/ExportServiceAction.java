...
public class ExportServiceAction extends WorkspaceJob {
	...
	public ExportServiceAction(String name, 
			...
			String jobVersion, 
			ServiceExportManager jobManager, IFile serviceWsdl, Map<String, String> operations) {
		super(name);
		this.nodes = nodes;
	...
	public IStatus runInWorkspace(IProgressMonitor arg0) throws CoreException {
		...
		for (RepositoryNode node : nodes) {
			...
			JobExportAction job = new JobExportAction(Arrays.asList(new RepositoryNode[]{node}), serviceVersion, manager, directoryName);
			try {
