...
public class CheckESBProviderNodes implements ICheckNodesService {
	...
	private static String COMPONENT_NAME = "tESBProviderRequest";
	...
	public void checkNode(Node node) {
		...
		List<? extends INode> nodesOfType = process
				.getNodesOfType(COMPONENT_NAME);
		if (nodesOfType.size() > 1) {
			Problems.add(ProblemStatus.ERROR, (Element) node, "Only one "
					+ COMPONENT_NAME + " component is allowed at one Job!");
		}
	}
