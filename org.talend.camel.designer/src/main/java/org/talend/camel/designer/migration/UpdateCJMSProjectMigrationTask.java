...
public class UpdateCJMSProjectMigrationTask extends AbstractItemMigrationTask {
	...
	private static final ProxyRepositoryFactory REPO_FACTORY = ProxyRepositoryFactory
			.getInstance();
	 ...
	 * @param eList
	 */
	private void addParamTtpe(NodeType nodeType, String field, String name,
			String value, EList eList) {
	...
	private NodeType createConnectionFactoryNode(NodeType currentNode) {
		...
		String label = getParameterValue("LABEL",
				currentNode.getElementParameter());
		if (label == null || label.equals("_UNIQUE_NAME_")) {
			label = getParameterValue("UNIQUE_NAME",
					currentNode.getElementParameter());
		...
		addParamTtpe(currentNode, "TEXT", "CONNECTION_FACOTRY_LABEL", label,
				null);
		addParamTtpe(currentNode, "TEXT", "CONNECTION_FACOTRY",
				label.replace("_", ""), null);

		for (Object e : currentNode.getElementParameter()) {
			ElementParameterType p = (ElementParameterType) e;
			if (!("UNIQUE_NAME".equals(p.getName())
					|| "LABEL".equals(p.getName())
					|| "TYPE".equals(p.getName()) || "DESTINATION".equals(p
					.getName()))) {

				addParamTtpe(nodeType, p.getField(), p.getName(), p.getValue(),
						p.getElementValue());
			}
		}
	 ...
	 */
	protected ElementParameterType createParamType(String field, String name,
			String value, EList<?> elementParameterTypes) {
		ElementParameterType paramType = FILE_FACTORY
	...
	protected String getParameterValue(String paramName,
			EList<?> elementParameterTypes) {
		ElementParameterType parameterType = findElementParameterByName(
				paramName, elementParameterTypes);
